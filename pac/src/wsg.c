...
void wsg_write(wsg *const w, int8_t address, uint8_t value)
{
	// waveform 1
	if (address == 0x5) {
		w->voices[0].waveform_no = value & 0x7 /* 0b111 */;
	} else if (address >= 0x10 && address <= 0x14) {
		uint8_t sample_no = address - 0x10;
		w->voices[0].frequency &= ~(0xF /* 0b1111 */ << (sample_no * 4));
		w->voices[0].frequency |= (value & 0xF /* 0b1111 */) << (sample_no * 4);
	} else if (address == 0x15) {
		w->voices[0].volume = value & 0xf;
	} else if (address >= 0 && address <= 0x4) {
		uint8_t sample_no = address - 0;
		w->voices[0].accumulator &= ~(0xF /* 0b1111 */ << (sample_no * 4));
		w->voices[0].accumulator |= (value & 0xF /* 0b1111 */) << (sample_no * 4);
	}
	// waveform 2
	else if (address == 0xa) {
		w->voices[1].waveform_no = value & 0x7 /* 0b111 */;
	} else if (address >= 0x16 && address <= 0x19) {
		// voice 2 and 3 cannot set lowest 4 bits of frequency
		uint8_t sample_no = address - 0x16 + 1;
		w->voices[1].frequency &= ~(0xF /* 0b1111 */ << (sample_no * 4));
		w->voices[1].frequency |= (value & 0xF /* 0b1111 */) << (sample_no * 4);
		...
		w->voices[1].accumulator &= ~(0xF /* 0b1111 */ << (sample_no * 4));
		w->voices[1].accumulator |= (value & 0xF /* 0b1111 */) << (sample_no * 4);
	}
	// waveform 3
	else if (address == 0xf) {
		w->voices[2].waveform_no = value & 0x7 /* 0b111 */;
	} else if (address >= 0x1b && address <= 0x1e) {
		// voice 2 and 3 cannot set lowest 4 bits of frequency
		uint8_t sample_no = address - 0x1b + 1;
		w->voices[2].frequency &= ~(0xF /* 0b1111 */ << (sample_no * 4));
		w->voices[2].frequency |= (value & 0xF /* 0b1111 */) << (sample_no * 4);
	} else if (address == 0x1f) {
		w->voices[2].volume = value & 0xf;
	} else if (address >= 0xb && address <= 0xe) {
		// voice 2 and 3 cannot set lowest 4 bits of accumulator
		uint8_t sample_no = address - 0xb + 1;
		w->voices[2].accumulator &= ~(0xF /* 0b1111 */ << (sample_no * 4));
		w->voices[2].accumulator |= (value & 0xF /* 0b1111 */) << (sample_no * 4);
	}
}
