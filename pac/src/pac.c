...
static void wb(void *userdata, uint16_t addr, uint8_t val)
{
    ...
    } else if (addr == 0x7ffd || addr == 0x7ffe) {
        p->ram[addr] = val; // in order to avoid write error messages
    } else {
        printf("ERR: write 0x%02x at 0x%04x\n", val, addr);
    }
}
...
{
    const size_t buf_size = strlen(s1) + strlen(s2) + 1;
...
static inline int load_file(const char *filename, uint8_t *memory, size_t nb_bytes)
{
...
static inline void decode_strip(uint8_t *const input, uint8_t *const output, int bx, int by, int img_width)
{
    int x, y, i;
    uint8_t strip;
...
static inline void preload_images(pac *const p)
{
    ...
    for (i = 0; i < NB_TILES; i++) {
        ...
        decode_strip(rom + 0, tile, 0, 4, TILE_WIDTH);
        decode_strip(rom + 8, tile, 0, 0, TILE_WIDTH);
    }
    ...
    for (i = 0; i < NB_SPRITES; i++) {
        ...
        decode_strip(rom + 0 * 8, sprite, 8, 12, SPRITE_WIDTH);
        decode_strip(rom + 1 * 8, sprite, 8, 0, SPRITE_WIDTH);
        decode_strip(rom + 2 * 8, sprite, 8, 4, SPRITE_WIDTH);
        decode_strip(rom + 3 * 8, sprite, 8, 8, SPRITE_WIDTH);

        decode_strip(rom + 4 * 8, sprite, 0, 12, SPRITE_WIDTH);
        decode_strip(rom + 5 * 8, sprite, 0, 0, SPRITE_WIDTH);
        decode_strip(rom + 6 * 8, sprite, 0, 4, SPRITE_WIDTH);
        decode_strip(rom + 7 * 8, sprite, 0, 8, SPRITE_WIDTH);
    }
}

static inline void draw_tile(pac *const p, uint8_t tile_no, uint8_t *pal, int16_t x, int16_t y)
{
    ...
    for (int i = 0; i < 8 * 8; i++) {
        ...
        get_color(p, pal[color], &p->screen_buffer[screenbuf_pos * 3 + 0], &p->screen_buffer[screenbuf_pos * 3 + 1], &p->screen_buffer[screenbuf_pos * 3 + 2]);
    }
}

static inline void draw_sprite(pac *const p, uint8_t sprite_no, uint8_t *pal, int16_t x, int16_t y, bool flip_x, bool flip_y)
{
...
static inline void pac_draw(pac *const p)
{
    ...
    const uint16_t VRAM_SCREEN_MID = VRAM_SCREEN_BOT + 64;
    const uint16_t VRAM_SCREEN_TOP = VRAM_SCREEN_MID + 0x380;
...
static inline void sound_update(pac *const p)
{
    ...
    for (int i = 0; i < p->sample_rate / PAC_FPS; i++) {
        int pos = (int) d * i;
        p->push_sample(p->audio_buffer[pos]);
    }
}

int pac_init(pac *const p, const char *rom_dir)
{
    char *load_path, *rom_files[] = {"pacman.6e", "pacman.6f", "pacman.6h", "pacman.6j", "82s123.7f", "82s126.4a", "pacman.5e", "pacman.5f", "82s126.1m", "82s126.3m", NULL};
    int i, r;
    uint16_t index;
    ...
    index = 0x0000;
    for (i = r = 0; i < 4; i++) {
        load_path = append_path(rom_dir, rom_files[i]);

        r += load_file(load_path, &p->rom[index], 0x1000);
        free(load_path);
        index += 0x1000;
    }

    load_path = append_path(rom_dir, rom_files[i++]);
    r += load_file(load_path, p->color_rom, 32);
    free(load_path);

    load_path = append_path(rom_dir, rom_files[i++]);
    r += load_file(load_path, p->palette_rom, 0x100);
    free(load_path);

    load_path = append_path(rom_dir, rom_files[i++]);
    r += load_file(load_path, p->tile_rom, 0x1000);
    free(load_path);

    load_path = append_path(rom_dir, rom_files[i++]);
    r += load_file(load_path, p->sprite_rom, 0x1000);
    free(load_path);

    load_path = append_path(rom_dir, rom_files[i++]);
    r += load_file(load_path, p->sound_rom1, 0x100);
    free(load_path);

    load_path = append_path(rom_dir, rom_files[i++]);
    r += load_file(load_path, p->sound_rom2, 0x100);
    free(load_path);
...
void pac_update(pac *const p, uint32_t ms)
{
    ...
    // so we need to execute (p->speed * ms * PAC_CLOCK_SPEED / 1000)
    unsigned long count = 0, cyc;
    const uint32_t speed = p->speed * ms * PAC_CLOCK_SPEED / 1000;
...
void pac_cheat_invincibility(pac *const p)
{
    ...
    invincibility = !invincibility; // toggle invincibility

    printf("%s", invincibility ? "invincibility patch ENABLED\n" : "invincibility patch DISABLED\n");
}
